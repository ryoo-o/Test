...
const syncVolume=()=>{if(!volume)return;const v=Math.max(0,Math.min(1,Number(volume.value)||0));volume.value=String(v);setProgress(volume,v*100)};
...
if(audio){
 audio.src=MUSIC_SOURCE;audio.preload='metadata';audio.volume=volumeLevel;audio.muted=volumeLevel<=0;
...
volume?.addEventListener('input',()=>{const v=Math.max(0,Math.min(1,Number(volume.value)||0));volumeLevel=v;if(audio){audio.volume=v;audio.muted=v===0}volume.value=String(v);syncVolume()});
