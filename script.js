...
const glow=$('.cursor-glow');addEventListener('pointermove',e=>{if(!glow)return;glow.style.left=e.clientX+'px';glow.style.top=e.clientY+'px'},{passive:true});
const reduced=matchMedia('(prefers-reduced-motion:reduce)').matches,videos=$$('video');
const observeVideo=(v,ratio=.05)=>{if(!v||!'IntersectionObserver'in window)return;const src=v.getAttribute('src')||v.querySelector('source')?.getAttribute('src');if(!src)return;v.dataset.src=src;v.removeAttribute('src');v.load();let loaded=false,visible=false;
const load=()=>{if(loaded)return;v.src=v.dataset.src;v.preload='metadata';v.load();loaded=true},unload=()=>{if(!loaded||visible)return;v.pause();v.removeAttribute('src');v.load();loaded=false};
const io=new IntersectionObserver(es=>{const e=es[0];visible=e.isIntersecting&&e.intersectionRatio>=ratio;if(visible){load();if(!document.hidden)v.play().catch(()=>{})}else v.pause();if(!e.isIntersecting&&!visible)unload()},{rootMargin:'300px 0px',threshold:[0,.05,.25,.5]});io.observe(v);
document.addEventListener('visibilitychange',()=>{if(document.hidden)v.pause();else if(visible){load();v.play().catch(()=>{})}})};
videos.forEach(v=>{v.muted=true;v.playsInline=true;observeVideo(v,v.classList.contains('hero-video')?.08:.05)});
const reveal=[...$$('main section'),...$$('.panel'),...$$('.gallery-item'),$('.message-card')].filter(Boolean);
reveal.forEach((e,i)=>{e.classList.add('reveal');e.style.setProperty('--reveal-delay',`${Math.min(i%5,4)*.06}s`)});
if('IntersectionObserver'in window&&!reduced){const io=new IntersectionObserver(es=>es.forEach(e=>e.target.classList.toggle('visible',e.isIntersecting)),{rootMargin:'0px 0px -8% 0px',threshold:.08});reveal.forEach(e=>io.observe(e))}else reveal.forEach(e=>e.classList.add('visible'));
...
const setupMedia=(box,src,o={})=>{if(!box||!src)return null;box.innerHTML='';const v=isVideo(src),m=document.createElement(v?'video':'img');m.src=src;if(v){m.autoplay=!!o.autoplay;m.muted=o.muted!==false;m.loop=!!o.loop;m.controls=!!o.controls;m.playsInline=true;m.preload='metadata'}else m.alt=o.alt||'';m.onerror=()=>box.innerHTML='<div class="media-fallback">Unable to load this media file.</div>';box.appendChild(m);if(v&&o.autoplay)m.play().catch(()=>{});return m};
...
let musicAvailable=false,shuffle=false,repeat=false,toastTimer;const MUSIC_SOURCE='assets/music.mp3';
...
if(audio){audio.src=MUSIC_SOURCE;audio.preload='metadata';audio.volume=.85;
...
audio.addEventListener('canplay',()=>musicAvailable=true);audio.addEventListener('error',resetPlayer);
...
$$('.modal-close').forEach(b=>b.addEventListener('click',closeAll));$$('.modal').forEach(m=>m.addEventListener('click',e=>{if(e.target===m)closeAll()}));document.addEventListener('keydown',e=>{if(e.key==='Escape')closeAll()});
