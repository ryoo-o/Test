...
const clamp=(v,a,b)=>Math.min(Math.max(v,a),b),reduced=()=>window.matchMedia?.("(prefers-reduced-motion: reduce)")?.matches===true;
...
const links=$$(".nav a[href^='#']"),sections=links.map(a=>$(a.getAttribute("href"))).filter(Boolean),setActive=id=>links.forEach(a=>a.classList.toggle("active",a.getAttribute("href")===`#${id}`));
if("IntersectionObserver"in window){const sectionObserver=new IntersectionObserver(es=>{const v=es.filter(e=>e.isIntersecting).sort((a,b)=>b.intersectionRatio-a.intersectionRatio)[0];if(v?.target.id)setActive(v.target.id)},{rootMargin:"-25% 0px -55% 0px",threshold:[0,.15,.35,.6]});sections.forEach(s=>sectionObserver.observe(s))}
$$(".nav a,.quick-links a,.brand").forEach(a=>a.addEventListener("click",e=>{const id=a.getAttribute("href");if(!id?.startsWith("#"))return;const t=$(id);if(!t)return;e.preventDefault();closeMenu();t.scrollIntoView({behavior:reduced()?"auto":"smooth",block:"start"});history.replaceState?.(null,"",id)}));
let shuffleOn=false,repeatOn=false;
const SRC="assets/music.mp3",showToast=(msg,persist=false)=>{if(!toast)return;toast.innerHTML=msg;toast.classList.add("show");clearTimeout(showToast.t);if(!persist)showToast.t=setTimeout(()=>toast.classList.remove("show"),4500)},hideToast=()=>{if(!toast)return;clearTimeout(showToast.t);toast.classList.remove("show")};
...
if(audio){
setSrc();audio.volume=clamp(Number(volume?.value??.3),0,1);
audio.addEventListener("loadedmetadata",()=>{hideToast();sync()});audio.addEventListener("canplay",hideToast);audio.addEventListener("timeupdate",sync);audio.addEventListener("durationchange",sync);audio.addEventListener("volumechange",sync);audio.addEventListener("play",updatePlay);audio.addEventListener("pause",updatePlay);
...
play?.addEventListener("click",toggleMusic);quickMusic?.addEventListener("click",toggleMusic);
musicNav?.addEventListener("click",()=>{toggleMusic();panel?.scrollIntoView({behavior:reduced()?"auto":"smooth",block:"center"})});
prev?.addEventListener("click",()=>{if(!audio)return;audio.currentTime=0;sync()});next?.addEventListener("click",()=>{if(!audio)return;audio.currentTime=0;playMusic()});
...
seek?.addEventListener("input",()=>seekTo(seek.value));volume?.addEventListener("input",()=>{if(audio)audio.volume=clamp(Number(volume.value),0,1);progress(volume,Number(volume.value)*100)});
...
promptPlay?.addEventListener("click",()=>{closeMusicPrompt();playMusic()});promptLater?.addEventListener("click",closeMusicPrompt);musicPrompt?.addEventListener("click",e=>{if(e.target===musicPrompt)closeMusicPrompt()});
addEventListener("keydown",e=>{if(e.key==="Escape"&&musicPrompt?.classList.contains("open"))closeMusicPrompt()});setTimeout(openMusicPrompt,500);
...
sendMessage?.addEventListener("click",()=>{if(!messageBox)return;const msg=messageBox.value.trim();if(!msg){if(messageStatus)messageStatus.textContent="Write a birthday wish for Ruby first ✦";messageBox.focus();return}try{const old=JSON.parse(localStorage.getItem("rubyBirthdayMessages")||"[]"),arr=Array.isArray(old)?old:[];arr.push({message:msg,createdAt:new Date().toISOString()});localStorage.setItem("rubyBirthdayMessages",JSON.stringify(arr.slice(-25)));messageBox.value="";updateCount();if(messageStatus)messageStatus.textContent="Ruby's birthday wish has been saved ✦"}catch{if(messageStatus)messageStatus.textContent="Your birthday wish is ready — browser storage is unavailable."}});
